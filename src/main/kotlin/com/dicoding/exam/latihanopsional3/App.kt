...
fun manipulateString(str: String, int: Int): String {
    ...
    if (containsDigit) {
        // ambil angkanya, ubah agar menjadi tipe data integer, lalu simpan dalam variabel numberInString
        val numberInString = str.filter { it.isDigit() }.toInt()
        // kalikan dengan angka yang diinput lalu simpan hasilnya dalam variabel result
        val result =numberInString * int
        // angka pada string diganti dengan hasil
        return str.replace(Regex("\\d+"), result.toString())
    }

    // jika tidak mengandung angka, langsung print string dengan angka
    return "$str$int"
}
